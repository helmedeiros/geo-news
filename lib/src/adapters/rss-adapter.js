...
var feedErrors = require('./feed-errors');

function canonicalId(portalId, link) {
  var stripped = (link || '')
    .replace(/^https?:\/\//, '')
    .replace(/[#?].*$/, '')
    .replace(/\/+$/, '');
  return portalId + ':' + stripped;
}
...
function create(deps) {
  ...
  function fetch(portal, callback) {
    httpClient.get(portal.rss, function (err, body) {
      if (err) {
        return callback(feedErrors.http('failed to fetch ' + portal.rss, err));
      }
      var entries;
      try {
        entries = parser.parse(body);
      } catch (parseErr) {
        return callback(feedErrors.parse('failed to parse ' + portal.rss, parseErr));
      }
      var items = entries.map(function (entry) {
        return {
          id: entry.id || canonicalId(portal.id, entry.link),
          title: entry.title,
          link: entry.link,
