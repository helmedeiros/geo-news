...
function create(deps) {
  ...
  function enrich(item, callback) {
    if (!extractor) { return callback(null, item); }
    extractor.extract(item, function (err, locations) {
      if (err) { return callback(err); }
      callback(null, newsItem.withLocations(item, locations));
    });
  }
  ...
  function saveAll(items, callback) {
    ...
    items.forEach(function (item) {
      enrich(item, function (enrichErr, enriched) {
        if (failed) { return; }
        if (enrichErr) { failed = true; return callback(enrichErr); }
        repository.save(enriched, function (saveErr) {
          if (failed) { return; }
